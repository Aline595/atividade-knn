...
########## ATIVIDADE AULA KNN ###########
"""
Adicione o atributo intenção de voto à base de 
dados referente à rede social de cientistas de
dados. Sorteie uma intenção de voto para cada 
usuário da rede. Aumente o tamanho da base de
modo que ela tenha 100 usuários.
"""
def adicionar_intencao_voto(qtde_usuarios_na_rede):
  intencao_voto = []
  for _ in range (quantidade_de_usuarios_na_rede):
    if (random.randint(0, 1) == 0):
      intencao_voto.append(('Bolsonaro'))
    else:
      intencao_voto.append(('Haddad'))
